...
class MultiLayerPerceptron:
    def __init__(self, layers, learning_rate, activator_function, activator_derivative=lambda x: 1,
                 optimizer="gradient", batch_size=1):
        self.layers = layers
    ...
    def update_weights_gradient_batch(self, l, accumulated_gradients):
        """Actualiza pesos usando gradientes acumulados de un batch"""
        self.weights[l] = np.array(self.weights[l]) - self.learning_rate * accumulated_gradients

    def update_weights_momentum_batch(self, l, accumulated_gradients):
        """Actualiza pesos usando gradientes acumulados con momentum"""
        # Actualizar velocidad con momentum
        self.velocity[l] = self.momentum * self.velocity[l] - self.learning_rate * accumulated_gradients
        # Actualizar pesos usando la velocidad
        self.weights[l] = np.array(self.weights[l]) + self.velocity[l]

    def update_weights_adam_batch(self, l, accumulated_gradients):
        """Actualiza pesos usando gradientes acumulados con ADAM"""
        self.t += 1

        # Actualizar momentos
        self.m[l] = self.beta1 * self.m[l] + (1 - self.beta1) * accumulated_gradients
        self.v[l] = self.beta2 * self.v[l] + (1 - self.beta2) * np.square(accumulated_gradients)
    ...
    def back_propagation_batch(self, batch_expected_outputs, batch_hidden_states, batch_activations):
        """Backpropagation para un batch completo, retorna gradientes acumulados"""
        batch_size = len(batch_expected_outputs)
        accumulated_gradients = [np.zeros_like(np.array(w)) for w in self.weights]
        
        for i in range(batch_size):
            expected_output = np.array(batch_expected_outputs[i])
            hidden_states = batch_hidden_states[i]
            ...
            for l in range(len(self.weights)):
                ...
                activation = np.append(activations[l], 1.0)
                weight_gradients = np.outer(delta, activation)
                accumulated_gradients[l] += weight_gradients
        
        # Promediar los gradientes acumulados
        for l in range(len(self.weights)):
            accumulated_gradients[l] /= batch_size
    ...
    def train(self, training_set, expected_outputs, epochs):
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(indices), self.batch_size):
                ...
                batch_inputs = [training_set[idx] for idx in batch_indices]
                batch_outputs = [expected_outputs[idx] for idx in batch_indices]
                
                # Forward propagation para el batch
                batch_hidden_states = []
                batch_activations = []
                ...
                for x in batch_inputs:
                    ...
                    # Calcular error para este ejemplo
                    output = activations[-1]
                    error += np.mean((np.array(batch_outputs[batch_inputs.index(x)]) - output) ** 2)
                
                # Backpropagation para el batch
                accumulated_gradients = self.back_propagation_batch(batch_outputs, batch_hidden_states, batch_activations)
                
                # Actualizar pesos usando los gradientes acumulados
                for l in range(len(self.weights)):
                    if self.optimizer == "adam":
                        self.update_weights_adam_batch(l, accumulated_gradients[l])
                    elif self.optimizer == "momentum":
                        self.update_weights_momentum_batch(l, accumulated_gradients[l])
                    else:  # gradient descent por defecto
                        self.update_weights_gradient_batch(l, accumulated_gradients[l])
